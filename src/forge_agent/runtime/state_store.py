"""StateStore — file-based pipeline run result persistence.

Moved from project/state_store.py (S2.6): run result persistence belongs
to the runtime layer. StateStore stores RunRecord (pipeline execution
results with agent_reports + chief_summary) as JSON files per project.

Distinct from TaskStore (runtime/store.py), which is the Protocol for
SQLite-based TaskRun persistence in the async hosting layer (S3).
"""
