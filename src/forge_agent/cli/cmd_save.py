...
def add(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        "--message", "-m", default="forge-agent: save generated agents", help="Commit message"
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    if not (gen_dir / ".git").exists():
        print(
            "Not a git repo. Use --init to create one, or run 'git init' manually in generated_agents/"
        )
        return 1
