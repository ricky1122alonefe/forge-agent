...
class TestSQLiteReportStore:
    ...
    def test_query_by_agent_id(self, store: SQLiteReportStore) -> None:
        store.insert(AgentReport(
            agent_id="stock.monitor", name="Stock", domain="stock",
            verdict=Verdict.LEAN_POSITIVE, confidence=0.8, risk=0.2,
            run_id="run_1", timestamp="2026-06-27T10:00:00+00:00",
        ))
        store.insert(AgentReport(
            agent_id="football.predictor", name="Football", domain="football",
            verdict=Verdict.LEAN_NEUTRAL, confidence=0.6, risk=0.3,
            run_id="run_2", timestamp="2026-06-27T11:00:00+00:00",
        ))
    ...
    def test_query_by_domain(self, store: SQLiteReportStore) -> None:
        store.insert(AgentReport(
            agent_id="a1", name="A1", domain="stock",
            verdict=Verdict.NEUTRAL, confidence=0.5, risk=0.5,
            run_id="r1", timestamp="2026-06-27T10:00:00+00:00",
        ))
        store.insert(AgentReport(
            agent_id="a2", name="A2", domain="football",
            verdict=Verdict.NEUTRAL, confidence=0.5, risk=0.5,
            run_id="r2", timestamp="2026-06-27T11:00:00+00:00",
        ))
    ...
    def test_query_by_verdict(self, store: SQLiteReportStore) -> None:
        store.insert(AgentReport(
            agent_id="a1", name="A1", domain="test",
            verdict=Verdict.LEAN_POSITIVE, confidence=0.8, risk=0.2,
            run_id="r1", timestamp="2026-06-27T10:00:00+00:00",
        ))
        store.insert(AgentReport(
            agent_id="a2", name="A2", domain="test",
            verdict=Verdict.LEAN_NEGATIVE, confidence=0.3, risk=0.7,
            run_id="r2", timestamp="2026-06-27T11:00:00+00:00",
        ))
    ...
    def test_query_with_limit(self, store: SQLiteReportStore) -> None:
        for i in range(10):
            store.insert(AgentReport(
                agent_id=f"agent_{i}", name=f"Agent {i}", domain="test",
                verdict=Verdict.NEUTRAL, confidence=0.5, risk=0.5,
                run_id=f"run_{i}", timestamp=f"2026-06-27T{10+i}:00:00+00:00",
            ))
    ...
    def test_summary(self, store: SQLiteReportStore) -> None:
        store.insert(AgentReport(
            agent_id="a1", name="A1", domain="stock",
            verdict=Verdict.LEAN_POSITIVE, confidence=0.8, risk=0.2, weight=1.0,
            run_id="r1", timestamp="2026-06-27T10:00:00+00:00",
        ))
        store.insert(AgentReport(
            agent_id="a2", name="A2", domain="stock",
            verdict=Verdict.LEAN_NEGATIVE, confidence=0.4, risk=0.6, weight=2.0,
            run_id="r2", timestamp="2026-06-27T11:00:00+00:00",
        ))
    ...
    def test_evidence_and_warnings_preserved(self, store: SQLiteReportStore, sample_report: AgentReport) -> None:
        store.insert(sample_report)
        result = store.get_by_run_id("run_001")
        assert result is not None
    ...
    def test_metrics_and_raw_preserved(self, store: SQLiteReportStore, sample_report: AgentReport) -> None:
        store.insert(sample_report)
        result = store.get_by_run_id("run_001")
        assert result is not None
